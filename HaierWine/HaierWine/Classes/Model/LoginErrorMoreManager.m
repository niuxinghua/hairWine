...
@implementation LoginErrorMoreManager
ITTOBJECT_SINGLETON_BOILERPLATE(LoginErrorMoreManager, sharedLoginErrorMoreManager);

- (BOOL)judgeByUser:(NSString *)user password:(NSString *)password currDate:(NSDate *)currDate
{
    UserNameAndPassword * model = [[UserNameAndPassword alloc]init];
    model.user = user;
    model.pwd = password;
    model.counter  = @"1";
...
{
    UserNameAndPassword * cacheModel = [[ITTDataCacheManager sharedManager] getCachedObjectByKey:KEY_CACHE_PREVENT_PWD_MORE];
    if (cacheModel == nil) {
